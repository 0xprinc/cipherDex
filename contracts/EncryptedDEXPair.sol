...
contract EncryptedDEXPair is EncryptedERC20 {
    ...
    function mint(address to, euint32 amount0, euint32 amount1) internal {
        ...
        if (totalSupply() == 0) {
            ...
            euint32 liquidity0 = TFHE.div(TFHE.mul(amount0, uint32(_totalSupply)), uint32(reserve0)); // to avoid overflows
            euint32 liquidity1 = TFHE.div(TFHE.mul(amount1, uint32(_totalSupply)), uint32(reserve1)); // to avoid overflows
            liquidity = TFHE.min(liquidity0, liquidity1); // this always fit in a euint32 from the logic of the contract
        }
    ...
    function claimBurn(uint256 tradingEpoch, address user) external {
        ...
        // ); 
        euint32 token0Claimable = TFHE.div(TFHE.mul(pendingBurn, uint32(totalToken0)), uint32(decryptedTotalBurn)); 
        // this always fit in a euint32
        // euint32 token1Claimable = TFHE.asEuint32(TFHE.div(TFHE.mul(TFHE.asEuint64(pendingBurn), uint64(totalToken1)), uint64(decryptedTotalBurn))); // this always fit in a euint32
        euint32 token1Claimable = TFHE.div(TFHE.mul(pendingBurn, uint32(totalToken1)), uint32(decryptedTotalBurn));
    ...
    function claimSwap(uint256 tradingEpoch, address user) external {
        ...
        if (totalToken1In != 0) {
            // amount0Out = TFHE.asEuint32(TFHE.div(TFHE.mul(TFHE.asEuint64(pending1In), uint64(totalToken0Out)), uint64(totalToken1In)));
            amount0Out = TFHE.div(TFHE.mul(pending1In, uint32(totalToken0Out)), uint32(totalToken1In));
            token0.transfer(user, amount0Out);
        }
        if (totalToken0In != 0) {
            // amount1Out = TFHE.asEuint32(TFHE.div(TFHE.mul(TFHE.asEuint64(pending0In), uint64(totalToken1Out)), uint64(totalToken0In)));
            amount1Out = TFHE.div(TFHE.mul(pending0In, uint32(totalToken1Out)), uint32(totalToken0In));
